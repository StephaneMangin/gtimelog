...
class TestSettings(unittest.TestCase):
    """Test core Settings infrastructure (path management, config file handling)."""
    ...
    def test_get_config_dir_1(self):
        ...
        assert self.settings.get_config_dir() == os.path.normpath('/tmp/home/.gt')

    def test_get_config_dir_2(self):
        # Case 2: ~/.gtimelog exists
        os.path.isdir = lambda _dir: True
        assert self.settings.get_config_dir() == os.path.normpath('/tmp/home/.gtimelog')

    def test_get_config_dir_3(self):
        # Case 3: ~/.gtimelog does not exist, so we use XDG
        os.path.isdir = lambda _dir: False
        assert self.settings.get_config_dir() == os.path.normpath('/tmp/home/.config/gtimelog')
    ...
    def test_get_config_dir_4(self):
        ...
        assert self.settings.get_config_dir() == os.path.normpath('/tmp/home/.conf/gtimelog')

    def test_get_data_dir_1(self):
        # Case 1: GTIMELOG_HOME is present in the environment
        os.environ['GTIMELOG_HOME'] = os.path.normpath('~/.gt')
        assert self.settings.get_data_dir() == os.path.normpath('/tmp/home/.gt')

    def test_get_data_dir_2(self):
        # Case 2: ~/.gtimelog exists
        os.path.isdir = lambda _dir: True
        assert self.settings.get_data_dir() == os.path.normpath('/tmp/home/.gtimelog')

    def test_get_data_dir_3(self):
        # Case 3: ~/.gtimelog does not exist, so we use XDG
        os.path.isdir = lambda _dir: False
        assert self.settings.get_data_dir() == os.path.normpath('/tmp/home/.local/share/gtimelog')

    def test_get_data_dir_4(self):
        # Case 4: XDG_CONFIG_HOME is present in the environment
        os.environ['XDG_DATA_HOME'] = os.path.normpath('~/.data')
        assert self.settings.get_data_dir() == os.path.normpath('/tmp/home/.data/gtimelog')

    def test_get_config_file(self):
        self.settings.get_config_dir = lambda: os.path.normpath('~/.config/gtimelog')
        assert self.settings.get_config_file() == os.path.normpath('~/.config/gtimelog/gtimelogrc')

    def test_load(self):
        # Base Settings can load config file (returns empty list from /dev/null)
        self.settings.load('/dev/null')
        # Base Settings has no domain-specific attributes

    def test_load_default_file(self):
        # Base Settings can attempt to load default config (may not exist)
        self.settings.load()

    def test_save(self):
        # Base Settings can save minimal config file
