...
from gtimelog.addons.base.helpers import mark_time
...
class TaskEntry(Gtk.Entry):
    ...
    completion_limit = GObject.Property(
        type=int, default=1000, nick='Completion limit', blurb='Maximum number of items in the completion popup'
    )

    gtk_completion_enabled = GObject.Property(
        type=bool, default=True, nick='Completion enabled', blurb='GTK+ completion enabled?'
    )
    ...
    def set_up_completion(self):
        ...
        completion.set_match_func(self.completion_match_func, self.completion_choices)
    ...
    def _split_correction(self, entry):
        ...
        suffix, _corrected_time = self.timelog.parse_correction(entry)
    ...
    def _do_history(self, delta):
        ...
        if self.history_pos == 0:
            ...
            self.filtered_history = uniq([entry for entry in self.history if entry.startswith(search_prefix)])
