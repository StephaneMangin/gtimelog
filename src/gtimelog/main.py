...
import gi
from gi.repository import Gdk, Gio, GLib, GObject, Gtk, Pango, Soup, Secret
...
class Application(Gtk.Application):
    ...
    def on_refresh_tasks(self, action, parameter):
        ...
        if gsettings.get_boolean('remote-jira-task'):
            if self.get_active_window() is not None:
...
class Window(Gtk.ApplicationWindow):
    ...
    def load_tasks(self, *args):
        ...
        if self.gsettings.get_boolean('remote-task-list'):
            ...
            tasks.add_filename(filename)
            self.download_tasks()
        else:
            tasks.remove_filename(Settings().get_task_list_cache_file)
        if self.gsettings.get_boolean('remote-jira-task'):
            filename = Settings().get_task_jira_cache_file()
            tasks.add_filename(filename)
            self.download_jira_tasks()
        else:
            tasks.remove_filename(Settings().get_task_jira_cache_file())

        mark_time("tasks loaded")
        if self.tasks:
            for filename in self.tasks.get_filenames():
...
class TaskListView(Gtk.TreeView):
    ...
    def tasks_changed(self, *args):
        ...
        if self.tasks is None:
            ...
            return
        for group_name, group_items in sorted(self.tasks.get_groups()):
